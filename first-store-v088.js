(() => {
  ...
  style.textContent = `
    ...
    .property.store-centered-v099 .sector-back{opacity:1!important;transform:translateY(0)!important;visibility:visible!important}
    .property.store-centered-v099 .sector-back .store-v088{opacity:1!important;visibility:visible!important}
  ...
  const placeholders = [...back.querySelectorAll('.sector-zone,.sector-crystal,.sector-title')];

  const isCenteredBack = () => {
    const text = (compass?.textContent || '').trim();
    const match = text.match(/BACK PROPERTY\s*•\s*(\d+)/i);
    if (!match) return false;
    return Math.abs(Number(match[1]) - 180) <= 1;
  };

  const setIsolation = (on) => {
    property?.classList.toggle('store-centered-v099', on);
    if (cameraStage) cameraStage.hidden = on;
    if (right) right.hidden = on;
    if (left) left.hidden = on;
    placeholders.forEach(el => el.hidden = on);
  };

  const updateButton = () => {
    const centered = !!property && !property.hidden && isCenteredBack();
    setIsolation(centered && panel.hidden);
  ...
  open.addEventListener('click', e => {
    e.preventDefault();
    panel.hidden = false;
    document.body.classList.add('store-panel-open-v088');
    setIsolation(false);
    updateButton();
  });
  panel.querySelector('.store-close-v088').addEventListener('click', e => {
    e.preventDefault();
    panel.hidden = true;
    document.body.classList.remove('store-panel-open-v088');
    updateButton();
  });
  ...
  const stamp = document.querySelector('.build-stamp'); if (stamp) stamp.textContent = 'AIC Living Site • v0.9.9';
})();
