...
private class LivingYardV5View(context: Context, private val onHotspot: (String) -> Unit) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (lifeOn) {
            drawAurora(canvas, t); drawSmoke(canvas, t); drawFire(canvas, t); drawPropertyLights(canvas, t); drawPeople(canvas, t); drawAnimalMotion(canvas, t); drawAuroraRoute(canvas, t); drawEagleMotion(canvas, t)
        }
    ...
    private fun drawAurora(canvas: Canvas, t: Float) {
        ...
        for (band in 0..3) {
            auroraPaint.color = colors[band]; auroraPaint.strokeWidth = 24f + band * 7f; auroraPaint.alpha = (78 + 36 * sin(t * 0.45f + band * 0.7f)).toInt().coerceIn(38, 125)
            val path = Path(); val start = worldPoint(-0.04f, 0.07f + band * 0.035f); path.moveTo(start.x, start.y)
            for (i in 1..30) {
                val nx = -0.04f + i / 28f; val wave = sin(nx * 10f + t * (0.46f + band * 0.04f)) + 0.45f * sin(nx * 19f - t * 0.31f); val breathe = 1f + 0.22f * sin(t * 0.22f + band)
                val p = worldPoint(nx, 0.075f + band * 0.038f + 0.028f * wave * breathe); path.lineTo(p.x, p.y)
            }
    ...
    private fun drawFire(canvas: Canvas, t: Float) {
        ...
        val flameH = 22f + 8f * sin(t * 12f); firePaint.color = Color.argb(220, 255, 205, 75); canvas.drawOval(RectF(p.x - 7f, p.y - flameH, p.x + 7f, p.y + 6f), firePaint)
        firePaint.color = Color.argb(210, 255, 85, 20); val sway = 7f * sin(t * 8f); canvas.drawOval(RectF(p.x - 4f + sway, p.y - flameH * 0.7f, p.x + 5f + sway, p.y + 5f), firePaint)
    }
    ...
    private fun drawPeople(canvas: Canvas, t: Float) {
        ...
        seats.forEachIndexed { index, seat ->
            ...
            lifePaint.color = Color.argb(170, 20 + index * 12, 16, 15); canvas.drawCircle(p.x + lean, p.y - 13f + bob, 6f, lifePaint); canvas.drawRoundRect(RectF(p.x - 6f + lean, p.y - 8f + bob, p.x + 7f + lean, p.y + 12f + bob), 5f, 5f, lifePaint)
            lifePaint.strokeWidth = 3f; lifePaint.style = Paint.Style.STROKE; canvas.drawLine(p.x, p.y + bob, p.x + 10f * sin(t * 0.55f + index), p.y + 7f + bob, lifePaint); lifePaint.style = Paint.Style.FILL
        }
    }
    ...
    private fun drawAuroraRoute(canvas: Canvas, t: Float) {
        ...
        lifePaint.shader = RadialGradient(p.x, p.y, 18f, intArrayOf(Color.argb(110, 255, 90, 185), Color.TRANSPARENT), null, Shader.TileMode.CLAMP); canvas.drawCircle(p.x, p.y, 18f, lifePaint); lifePaint.shader = null
        lifePaint.color = Color.argb(190, 255, 130, 205); canvas.drawCircle(p.x - 5f, p.y, 3f, lifePaint); canvas.drawCircle(p.x + 5f, p.y, 3f, lifePaint); canvas.drawCircle(p.x, p.y - 5f, 3f, lifePaint); canvas.drawOval(RectF(p.x - 5f, p.y + 2f, p.x + 5f, p.y + 10f), lifePaint)
    }
    ...
    private fun drawEagleMotion(canvas: Canvas, t: Float) {
        ...
        lifePaint.color = Color.argb(100, 5, 10, 15); lifePaint.style = Paint.Style.STROKE; lifePaint.strokeWidth = 4f; val flap = 11f + 7f * sin(t * 3.2f)
        canvas.drawLine(p.x, p.y, p.x - 20f, p.y - flap, lifePaint); canvas.drawLine(p.x, p.y, p.x + 20f, p.y - flap, lifePaint); lifePaint.style = Paint.Style.FILL
    }

    private fun drawSnow(canvas: Canvas, t: Float) {
        for (f in flakes) {
            val y = ((f.y + t * f.speed + f.phase) % 1.08f) * height; val x = f.x * width + sin(t * 0.7f + f.phase * 5f) * 18f
