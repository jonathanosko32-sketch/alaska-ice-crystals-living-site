(function oskoFix83PageGlideControls(){
'use strict';
if(window.__oskoFix83)return;window.__oskoFix83=true;
...
css.id='hq83-style';
css.textContent=`
/* FIX83 — slower two-stage glide, soft landing, and owner page controls. */
.bible66.fix83 .h74-book{touch-action:pan-y!important;overflow:hidden!important;perspective:1500px!important;transform-style:preserve-3d!important;width:var(--h83-book-width,100%)!important;max-width:none!important;margin-inline:auto!important;transition:width .42s ease!important}
.bible66.fix83 .h74-page{touch-action:pan-y!important}
.bible66.fix83 .h79-swipe-help{display:none!important}
...
.bible66.fix83.h82-turning .h74-turn{opacity:0!important}
.bible66.fix83.h82-turning .h82-corner,.bible66.fix83.h82-pinching .h82-corner{opacity:.18}
.bible66.fix83.h82-pinching .h74-book{filter:drop-shadow(0 0 14px #56dbff88)}
.h83-page-controls{margin:8px 0 10px;padding:8px;border:2px solid #b7894d;border-radius:12px;background:linear-gradient(180deg,#3c1e0e,#1b0c05);box-shadow:inset 0 0 15px #0008;display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:7px}
.h83-page-controls button{min-height:58px;padding:7px 4px;border:2px solid #b7894d;border-radius:10px;background:linear-gradient(#68401f,#321707);color:#fff7df;font:900 11px/1.05 system-ui;text-shadow:0 2px 2px #000;touch-action:manipulation}
.h83-page-controls button:active{transform:translateY(2px);filter:brightness(1.25)}
.h83-page-readout{grid-column:1/-1;min-height:30px;display:grid;place-items:center;border-radius:8px;background:#07364a;color:#d8f9ff;font:900 10px system-ui;letter-spacing:.06em}

/* Carry FIX81's finished HQ rooms forward without loading FIX81's flat corner gesture. */
#hq66.fix83 .h81-hq-room{position:relative;overflow:hidden;min-height:520px;margin-top:24px;padding:24px;border:8px solid #30170a;border-radius:25px;background:linear-gradient(155deg,#98623bee,#5b3119f4 62%,#31170bed);box-shadow:0 28px 42px #000c,inset 0 0 42px #170803,0 0 0 2px #c28c5544}
#hq66.fix83 .h81-hq-room:after{content:'';position:absolute;inset:12px;border:1px solid #e0b06b3b;border-radius:15px;pointer-events:none}
#hq66.fix83 .h81-hq-room h2{position:relative;z-index:4;margin:0;text-align:center;color:#ffe2aa;font:900 28px Georgia;letter-spacing:.08em;text-shadow:0 3px 5px #000}
...
@media(max-width:520px){#hq66.fix83 .h81-hq-room{min-height:620px}.h81-map,.h81-console{width:88%;left:6%;right:auto}.h81-map{top:145px}.h81-console{top:350px}.h81-cabinets{display:none}.h82-corner{width:58px;height:58px}.h83-page-controls{grid-template-columns:repeat(3,minmax(0,1fr))}.h83-page-controls button{min-height:62px;font-size:10px}.h83-page-readout{grid-column:1/-1}}
`;
document.head.appendChild(css);

if(bible&&book&&next&&prev){
 bible.classList.remove('fix81','fix82');bible.classList.add('fix83');
 const controls=document.createElement('div');controls.className='h83-page-controls';controls.setAttribute('aria-label','Bible page controls');
 controls.innerHTML='<button id="h83-glide-slower">GLIDE<br>SLOWER</button><button id="h83-glide-faster">GLIDE<br>FASTER</button><button id="h83-book-narrow">BOOK<br>SMALLER</button><button id="h83-book-normal">NORMAL<br>BOOK</button><button id="h83-book-wide">BOOK<br>WIDER</button><output class="h83-page-readout" id="h83-page-readout"></output>';
 book.parentNode.insertBefore(controls,book);
 ...
 let bookWidth=clamp(Number(localStorage.getItem('osko_bible_book_width'))||100,92,108);
 ...
 const readout=document.getElementById('h83-page-readout');
 function showPageControls(save){
  bible.style.setProperty('--h83-book-width',bookWidth+'%');
  if(readout)readout.textContent='PAGE GLIDE '+(glideMs/1000).toFixed(2)+' SEC • BOOK WIDTH '+bookWidth+'%';
  if(save)try{localStorage.setItem('osko_bible_glide_ms',String(glideMs));localStorage.setItem('osko_bible_book_width',String(bookWidth))}catch(_){}
 }
 ...
 function renderCurl(p,y,landing=0){
  ...
  strips.forEach((s,i)=>{
   ...
   const flatten=1-clamp(landing,0,1)*.96;
   const roll=Math.sin(turn*Math.PI)*Math.min(32,pageW*.09)*(0.35+progress*.65)*flatten;
   const slide=sideSign*(globalPull*pageU+turn*stripW*1.15+landing*pageW*.075*(1-pageU*.30));
   const lift=(dragY*pageU*(.16+.50*progress)-Math.sin(pageU*Math.PI)*5*progress)*flatten;
   s.style.transformOrigin=(corner==='right'?'0':'100%')+' 8%';
   s.style.transform='translate3d('+slide.toFixed(2)+'px,'+lift.toFixed(2)+'px,'+roll.toFixed(2)+'px) rotateY('+angle.toFixed(2)+'deg)';
  ...
  shadow.style.left=(parseFloat(mesh.style.left)+contact-19)+'px';shadow.style.top=mesh.style.top;shadow.style.height=pageH+'px';shadow.style.opacity=String(Math.min(.72,.12+progress*.72)*(1-landing*.92));shadow.style.transform='translateX('+(sideSign*(globalPull*.35+landing*pageW*.075))+'px) skewY('+(sideSign*progress*4*(1-landing))+'deg)';
  ...
  grab.style.transform='translate3d('+(edgeX-21)+'px,'+(edgeY-15)+'px,70px) rotate('+(sideSign*(18+progress*55))+'deg) scale('+(0.72+progress*.32)+')';
 }
 function clearMesh(){
 ...
 controls.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;
  if(b.id==='h83-glide-slower')glideMs=clamp(glideMs+250,900,2600);
  if(b.id==='h83-glide-faster')glideMs=clamp(glideMs-250,900,2600);
  if(b.id==='h83-book-narrow')bookWidth=clamp(bookWidth-4,92,108);
  if(b.id==='h83-book-normal')bookWidth=100;
  if(b.id==='h83-book-wide')bookWidth=clamp(bookWidth+4,92,108);
  showPageControls(true);if(status)status.textContent='PAGE CONTROLS SAVED • GLIDE '+(glideMs/1000).toFixed(2)+' SEC • WIDTH '+bookWidth+'%';
 });
 currentFont=getFont();showFont(currentFont,false);showPageControls(false);
}

if(root){
 root.classList.remove('fix81','fix82');root.classList.add('fix83');
