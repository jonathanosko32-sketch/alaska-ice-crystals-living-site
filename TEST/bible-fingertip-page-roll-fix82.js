(function oskoFix82FingertipPageRoll(){
'use strict';
if(window.__oskoFix82)return;window.__oskoFix82=true;
...
css.id='hq82-style';
css.textContent=`
/* FIX82 — fingertip-following paper. The page is a flexible strip mesh, not a flat flip. */
.bible66.fix82 .h74-book{touch-action:pan-y!important;overflow:hidden!important;perspective:1500px!important;transform-style:preserve-3d!important}
.bible66.fix82 .h74-page{touch-action:pan-y!important}
.bible66.fix82 .h79-swipe-help{display:none!important}
...
.bible66.fix82.h82-turning .h74-turn{opacity:0!important}
.bible66.fix82.h82-turning .h82-corner,.bible66.fix82.h82-pinching .h82-corner{opacity:.18}
.bible66.fix82.h82-pinching .h74-book{filter:drop-shadow(0 0 14px #56dbff88)}

/* Carry FIX81's finished HQ rooms forward without loading FIX81's flat corner gesture. */
#hq66.fix82 .h81-hq-room{position:relative;overflow:hidden;min-height:520px;margin-top:24px;padding:24px;border:8px solid #30170a;border-radius:25px;background:linear-gradient(155deg,#98623bee,#5b3119f4 62%,#31170bed);box-shadow:0 28px 42px #000c,inset 0 0 42px #170803,0 0 0 2px #c28c5544}
#hq66.fix82 .h81-hq-room:after{content:'';position:absolute;inset:12px;border:1px solid #e0b06b3b;border-radius:15px;pointer-events:none}
#hq66.fix82 .h81-hq-room h2{position:relative;z-index:4;margin:0;text-align:center;color:#ffe2aa;font:900 28px Georgia;letter-spacing:.08em;text-shadow:0 3px 5px #000}
...
@media(max-width:520px){#hq66.fix82 .h81-hq-room{min-height:620px}.h81-map,.h81-console{width:88%;left:6%;right:auto}.h81-map{top:145px}.h81-console{top:350px}.h81-cabinets{display:none}.h82-corner{width:58px;height:58px}}
`;
document.head.appendChild(css);

if(bible&&book&&next&&prev){
 bible.classList.remove('fix81');bible.classList.add('fix82');
 ...
 function renderCurl(p,y){
  ...
  strips.forEach((s,i)=>{
   ...
   const roll=Math.sin(turn*Math.PI)*Math.min(32,pageW*.09)*(0.35+progress*.65);
   const slide=sideSign*(globalPull*pageU+turn*stripW*1.15);
   const lift=dragY*pageU*(.16+.50*progress)-Math.sin(pageU*Math.PI)*5*progress;
  ...
  shadow.style.left=(parseFloat(mesh.style.left)+contact-19)+'px';shadow.style.top=mesh.style.top;shadow.style.height=pageH+'px';shadow.style.opacity=String(Math.min(.72,.12+progress*.72));shadow.style.transform='translateX('+(sideSign*globalPull*.35)+'px) skewY('+(sideSign*progress*4)+'deg)';
 ...
 function settle(commit){
  const from=progress,to=commit?1:0,fromY=dragY,duration=commit?Math.max(360,760-progress*300):260,start=performance.now(),side=corner;
  mode='animating';
  function frame(now){const t=clamp((now-start)/duration,0,1),e=commit?1-Math.pow(1-t,3):t*t*(3-2*t);renderCurl(from+(to-from)*e,fromY*(1-e));if(t<1){anim=requestAnimationFrame(frame);return}
...
if(root){
 root.classList.remove('fix81');root.classList.add('fix82');
