...
function oskoBuildingRoom(id,key,label){ptype.textContent='BUILDING SECTION';ptitle.textContent=oskoBuildingMenus[id].label+' • '+key;ptext.textContent=label+' is selected inside the Living OS. This section is a navigation target ready for its working tools and content.';actions.innerHTML='';const back=document.createElement('button');back.className='chip';back.textContent='BACK';back.onclick=()=>oskoOpenBuilding(id);actions.appendChild(back);oskoBuildingStatus(id,key+' SELECTED')}
...
\`;
    if(!html.includes(v15ActionMarker))throw new Error('building action marker missing');
    html=html.replace(v15ActionMarker,v15Helpers+'\\n'+v15ActionMarker+"if(oskoBuildingMenus[id]&&a==='OPEN'){oskoOpenBuilding(id);return}if(oskoBuildingMenus[id]&&a==='LIGHTS'){oskoToggleBuildingLights(id);return}");
    html=html.replace("status.textContent='READY • LIVING OS • PROPERTY SYSTEMS ONLINE';","status.textContent='READY • BUILDING SYSTEMS v15 • FIX8 DAY/NIGHT • LIVING OS ONLINE';");
`;
  wrapper=wrapper.replace(hook,inject+'\n'+hook);
  return wrapper;
}
window.OSKOBuildingSystemsV15={apply};
})();
