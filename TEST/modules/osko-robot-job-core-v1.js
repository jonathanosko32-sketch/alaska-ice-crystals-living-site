...
function create(opts){
  ...
  function dispatchNext(context){
    const actor=(context&&context.actor)||'skie';
    const decision=checkPermission(actor,'robot.job.dispatch',context||{});
    if(decision.allowed===false) return {ok:false,error:'PERMISSION_DENIED',decision};
  ...
  function finish(jobId,result,context){
    const actor=(context&&context.actor)||'skie';
  ...
  function cancel(jobId,reason,context){
    const actor=(context&&context.actor)||'skie';
    const decision=checkPermission(actor,'robot.job.cancel',context||{});
    if(decision.allowed===false) return {ok:false,error:'PERMISSION_DENIED',decision};
    const job=jobs.get(String(jobId)); if(!job) return {ok:false,error:'UNKNOWN_JOB'};
    ...
    if(job.state==='running'&&fleet&&job.assignedRobot&&typeof fleet.completeJob==='function') fleet.completeJob(job.assignedRobot,{ok:false,cancelled:true,reason:reason||null},{actor:actor,userConfirmed:!!(context&&context.userConfirmed),local:!!(context&&context.local)});
